...
namespace ma.Providers
{
    public class MongoDBSimpleMembershipProvider : ExtendedMembershipProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            ApplicationName = GetValueOrDefault(config, "applicationName", o => o.ToString(), "devnull_aspnetdb");

            // MongoDB setup
            var uri = ConfigurationManager.ConnectionStrings["antfarm"].ConnectionString;

            var mongoClient = new MongoClient(uri);
            mongoDB = mongoClient.GetServer().GetDatabase(new MongoUrl(uri).DatabaseName);
        ...
        {
            var acc = this.mongoDB.GetCollection<WebpagesMembership>("WebpagesMembership").FindOne(Query.EQ("_id", userId));
            return acc != null;
        }
        ...
        private void CreateUser(string userName, IDictionary<string, object> values)
        {
            ...
            if (values != null)
            {
                foreach (var value in values)
                {
                    ...
                    if (field != null)
                    {
                        user.GetType().GetProperty(field.Name).SetValue(user, value.Value, null);
                    }
                }
            }
